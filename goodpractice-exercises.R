...
co2 %>% group_by(Month) %>%
  ggplot() + 
  geom_line(aes(x = Year, y = co2, linetype=Month)) 
...
co2 %>% group_by(Month) %>%
  ggplot() + 
  geom_line(aes(x = Year, y = co2, linetype=Month)) + coord_flip()
...
plot1<-co2 %>% group_by(Month) %>%
  ggplot(aes(x = Year, y = co2, col=Month)) + 
  geom_line() + labs(x='Year', y='CO2', title='C02 concentration per year ', subtitle = 'Values for all months')
...
diamonds %>%
  ggplot(aes(x = cut_interval(price, n = 5))) + 
  geom_bar() +labs(x='diamond cut intervals', y= 'price per cut interval',
                   title='Diamond prices per cut interval')
...
diamonds %>% sample_n(10) %>%
  ggplot(aes(x = carat, y = price)) + 
  geom_point() +
  geom_label(aes(label=max(price)), nudge_x = .75)
